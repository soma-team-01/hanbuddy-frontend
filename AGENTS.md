...
- 배포는 SOMA 팀 Vercel Team `soma-zeroone` 소속으로 동작 중(PR #14~#20 배포에서 확인). 과거에는 개인(Hobby) 팀 `minbros-projects`(`team_5xpcSletzWjNA6mWCYKBd4qG`) 소속이었고, 팀이 바뀐 뒤에도 projectId는 동일하다.
...
- 팀 전환 관련 확인 필요 항목: 새 team ID 문서화, 팀원 초대 상태, 팀 플랜 비용/크레딧.
