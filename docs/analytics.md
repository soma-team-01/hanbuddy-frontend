...
The approved choice applies to this browser, including anonymous detail/CTA and login continuity. It is not an account-wide ledger. The implemented counterpart is backend commit `1174af4217f62ff2dbca09951c5305d1ea02f7d8`, documented in its `docs/analytics-purchase-outbox.md` and registered `shared-handoff-207.txt`. Earlier `/me/analytics-consent` and browser-ledger proposals are superseded.
...
Frontend identifier lookup/linkage is fire-and-forget and cannot delay or reject the booking result. Missing/invalid consent simply omits capture context. **Backend limitation:** the valid pre-PG analytics capture participates in payment preparation's database transaction; a capture persistence failure can roll that transaction back before PG. Therefore end-to-end independence of booking from every analytics failure is not proven. Hermes must relay this backend availability issue before activation. Backend code is not changed here.
