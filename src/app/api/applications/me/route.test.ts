...
describe("GET /api/applications/me", () => {
  ...
  it("returns 401 without calling the backend when the access token cookie is missing", async () => {
    const response = await GET(new NextRequest("http://localhost/api/applications/me"));

    expect(response.status).toBe(401);
    expect(mockedGetBackend).not.toHaveBeenCalled();
  });

  it("proxies my applications with the access token as bearer", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/applications/me", {
        headers: { cookie: `${AUTH_COOKIES.accessToken}=access-token` },
      }),
    );

    expect(mockedGetBackend).toHaveBeenCalledWith("/applications/me", {
