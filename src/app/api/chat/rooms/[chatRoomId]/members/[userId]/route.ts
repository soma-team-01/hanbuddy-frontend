...
interface ChatMemberRouteContext {
  params: Promise<{ chatRoomId: string; userId: string }>;
}

export async function DELETE(request: NextRequest, context: ChatMemberRouteContext) {
  const { chatRoomId, userId } = await context.params;
  if (!isValidChatRoomId(chatRoomId) || !isValidChatRoomId(userId)) {
    return badRequestResponse("잘못된 참여자 요청입니다.");
  }

  return proxyAuthenticatedDelete<null>(
    request,
    `/chat/rooms/${chatRoomId}/members/${userId}`,
    "채팅 서버에 연결할 수 없습니다.",
  );
}
