 ...
 * preview면(버디 미리보기) 화면은 동일하되 예약 단계로 이동할 수 없다.
 */
export function ActivityDetailView({
  activity,
  preview = false,
}: Readonly<{ activity: Activity; preview?: boolean }>) {
        ...
        <main data-testid="activity-detail-layout" className="mx-auto w-full max-w-[840px]">
          <article className="min-w-0">
            <div className="relative grid h-[320px] grid-cols-2 gap-2 overflow-hidden rounded-3xl md:h-[420px] md:grid-cols-[1.4fr_0.6fr]">
              ...
              {activity.isSoldOut ? (
                <span className="absolute top-4 left-4 rounded-full bg-ink/80 px-3 py-1.5 font-display text-xs font-bold text-white backdrop-blur-[2px]">
                  {tExplore("soldOut")}
                </span>
              ) : hasDiscount && activity.discountPercent ? (
                <span className="absolute top-4 left-4 rounded-full bg-primary px-3 py-1.5 font-display text-xs font-bold text-on-primary shadow-[0_6px_16px_rgba(209,63,50,0.35)]">
            ...
            <div className="flex flex-col gap-10 py-8 md:py-10">
              ...
              {itinerary.length > 0 ? (
                <section className="flex flex-col gap-6 border-t border-line-soft pt-6">
              ...
              <section className="flex flex-col gap-4 border-t border-line-soft pt-6">
                ...
                <div className="flex flex-col gap-4 rounded-3xl border border-line-soft bg-panel-raised p-5 sm:flex-row sm:gap-5 md:p-6">
      ...
      >
        <div className="mx-auto flex w-full max-w-[840px] items-center gap-3">
          <button
            ...
            className={`flex h-12 min-w-0 items-center gap-2 rounded-xl border border-line-strong bg-canvas-soft px-4 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
              selectedSession
