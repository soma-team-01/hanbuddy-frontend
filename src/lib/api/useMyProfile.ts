...
import { isUnauthenticatedError, toApiClientError } from "./errors";
...
export function useMyProfile(): MyProfileResult | null {
  ...
  if (profileQuery.isPending || isUnauthenticatedError(profileQuery.error)) {
    return null;
  }
  if (profileQuery.error) {
    return {
      status: "error",
      error: toApiClientError(profileQuery.error),
